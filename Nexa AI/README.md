...
The `infrastructure/requirements.txt` contains:
...
#### 4. Prepare the Lambda deployment package

The backend uses external Python dependencies such as FastAPI, LangChain, LangGraph and Pydantic. These dependencies must be included in the Lambda deployment package.
...
#### `CannotFindAsset ... backend\package`

The Lambda package does not exist at the path expected by CDK. Recreate it and install the backend dependencies:
...
Then make sure CDK uses:

```python
code=lambda_.Code.from_asset("../backend/package")
```

#### `ENOTEMPTY ... jsii-kernel`

This can occur during JSII/CDK temporary-directory cleanup on Windows. Close unnecessary terminals/processes and retry the deployment. If needed, clean the `jsii-kernel-*` directories under the Windows temporary directory and run `cdk deploy` again.
